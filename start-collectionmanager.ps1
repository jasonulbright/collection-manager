...
# =============================================================================
# Theme setup and toggle. Palette, sidebar, title bar, dialogs: SuiteCommon.
# =============================================================================
[void][ControlzEx.Theming.ThemeManager]::Current.ChangeTheme($window, 'Dark.Steel')

# Show-OptionsDialog (below) still reads these for its own inline light-theme
# title bar branch; not consumed by any function this migration deletes.
...
$btnRemoveRule.Add_Click({
    ...
    if (-not (Show-ConfirmDialog -Title 'Remove Rule' -Message ("Remove query rule '$name' from '$($coll.Name)'?") -Owner $window)) {
        return
    }
    try {
...
'@
    ...
    $btnRemoveSel.Add_Click({
        ...
        if (-not (Show-ConfirmDialog -Title 'Remove Members' -Message ("Remove $(@($rows).Count) selected member(s) from $($Collection.Name)?") -Owner $window)) {
            return
        }
        foreach ($r in $rows) {
            try {
...
$btnRemoveColl.Add_Click({
    ...
    if (-not (Show-ConfirmDialog -Title 'Remove Collection' -Message ("Permanently delete collection '$($row.Name)' ($($row.CollectionID))?") -Owner $window)) {
        return
    }
    try {
...
# =============================================================================
# Window state persistence (WinForms->WPF schema bridge, geometry: SuiteCommon).
# =============================================================================
